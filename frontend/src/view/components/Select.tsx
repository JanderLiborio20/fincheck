    ...
    <div>
      <div className="relative">
        <label
          ...
          text-gray-700`,
            selectedValue &&
              'text-xs left-[13px] top-2 translate-all translate-y-0',
          )}
        >
          {placeholder}
        </label>
            ...
            <RdxSelect.Content className="z-[52] overflow-hidden bg-white rounded-2xl border border-gray-100 shadow-[0px_11px_20px_0px_rgba(0,0,0,0.10)]">
              ...
              <RdxSelect.Viewport className="p-2">
                {options.map((option) => (
                  <RdxSelect.Item
                    key={option.value}
                    className="p-2 text-gray-800 text-sm cursor-pointer data-[state=checked]:font-bold outline-none data-[highlighted]:bg-gray-50 rounded-lg transition-colors"
                    value={option.value}
                  >
      ...
      {error && (
        <div className="flex gap-2 text-red-900 mt-2 items-center">
          <CrossCircledIcon />
